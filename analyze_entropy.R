...
apply_spanorm_normalization <- function(seurat_obj, sample_p = 0.1, chunk_size = 1000) {
  if (!requireNamespace("SpaNorm", quietly = TRUE)) {
    warning("SpaNorm package missing. Skipping SpaNorm step.")
    return(seurat_obj)
  }
  ...
  cat("Fitting SpaNorm model (may take a moment)...\n")
  ...
  cat("Normalizing data in chunks to conserve memory...\n")
  ...
  tryCatch({
    ...
    seurat_obj <- Seurat::SetAssayData(seurat_obj, assay = "Spatial", slot = "data", new.data = norm_sparse)
  })
